"""CI proxy for the 32-channel × 600 s validation.

Runs 30 s instead of 600 s — enough latency samples to confirm NFR-1 holds
at 32 channels while keeping CI runtime under a minute. The full 10-minute
operator run is `scripts/stress-multichannel.py --channels 32`.

Marked `slow`: skip locally with `pytest -m 'not slow'`.
"""
...
# Ports disjoint from all other stress / bridge tests.
_ZMQ_BIND = "tcp://127.0.0.1:55890"
_MCAST_IP = "239.255.99.5"
_MCAST_PORT = 55005
_METRICS_BIND = "127.0.0.1:55184"
...
@pytest.mark.slow
async def test_stress_32ch_30s() -> None:
